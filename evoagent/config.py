...
def _getenv(name: str, default=None):
    """Read TraceReview variables first while keeping EVOAGENT_* compatibility."""
    if name.startswith("EVOAGENT_"):
        alias = "TRACEREVIEW_" + name[len("EVOAGENT_"):]
        return os.getenv(alias, os.getenv(name, default))
    return os.getenv(name, default)


def _int(name: str, default: int) -> int:
    value = int(_getenv(name, str(default)))
    if value <= 0:
...
def _bool(name: str, default: bool = False) -> bool:
    return _getenv(name, str(default)).strip().lower() in {"1", "true", "yes", "on"}
...
class Settings:
    ...
    openrouter_app_name: str = "TraceReview"
    ...
    otel_endpoint: str = ""
    otel_service_name: str = "tracereview"
    ...
    enabled_agents: str = "coordinator,boundary-inspector,behavior-inspector,evidence-auditor"
    ...
    def from_env(cls) -> "Settings":
        return cls(
            host=_getenv("EVOAGENT_HOST", "127.0.0.1"),
            port=_int("EVOAGENT_PORT", 8080),
            db_path=_getenv("EVOAGENT_DB_PATH", "evoagent.db"),
            ...
            llm_base_url=_getenv("EVOAGENT_LLM_BASE_URL", "").rstrip("/"),
            llm_api_key=_getenv("EVOAGENT_LLM_API_KEY", ""),
            llm_model=_getenv("EVOAGENT_LLM_MODEL", ""),
            github_webhook_secret=_getenv("EVOAGENT_GITHUB_WEBHOOK_SECRET", ""),
            github_token=_getenv("EVOAGENT_GITHUB_TOKEN", ""),
            auto_post_review=_bool("EVOAGENT_AUTO_POST_REVIEW"),
            database_url=_getenv("EVOAGENT_DATABASE_URL", ""),
            redis_url=_getenv("EVOAGENT_REDIS_URL", ""),
            ...
            skills_dir=_getenv("EVOAGENT_SKILLS_DIR", "skills"),
            github_app_id=_getenv("EVOAGENT_GITHUB_APP_ID", ""),
            github_app_slug=_getenv("EVOAGENT_GITHUB_APP_SLUG", ""),
            github_private_key_path=_getenv("EVOAGENT_GITHUB_PRIVATE_KEY_PATH", ""),
            public_base_url=_getenv("EVOAGENT_PUBLIC_BASE_URL", "http://127.0.0.1:8080").rstrip("/"),
            llm_provider=_getenv("EVOAGENT_LLM_PROVIDER", "local"),
            deepseek_api_key=_getenv("EVOAGENT_DEEPSEEK_API_KEY", ""),
            openrouter_api_key=_getenv("EVOAGENT_OPENROUTER_API_KEY", ""),
            openrouter_site_url=_getenv("EVOAGENT_OPENROUTER_SITE_URL", ""),
            openrouter_app_name=_getenv("EVOAGENT_OPENROUTER_APP_NAME", "TraceReview"),
            eval_max_cases=_int("EVOAGENT_EVAL_MAX_CASES", 5),
            eval_min_cases=_int("EVOAGENT_EVAL_MIN_CASES", 3),
            eval_min_improvement=float(_getenv("EVOAGENT_EVAL_MIN_IMPROVEMENT", "0.01")),
            eval_min_holdout_cases=_non_negative_int("EVOAGENT_EVAL_MIN_HOLDOUT_CASES", 2),
            eval_max_metric_regression=float(
                _getenv("EVOAGENT_EVAL_MAX_METRIC_REGRESSION", "0")
            ),
            auth_required=_bool("EVOAGENT_AUTH_REQUIRED", False),
            auth_secret=_getenv("EVOAGENT_AUTH_SECRET", ""),
            bootstrap_admin_username=_getenv("EVOAGENT_BOOTSTRAP_ADMIN_USERNAME", ""),
            bootstrap_admin_password=_getenv("EVOAGENT_BOOTSTRAP_ADMIN_PASSWORD", ""),
            default_tenant_id=_getenv("EVOAGENT_DEFAULT_TENANT_ID", "default"),
            ...
            repair_test_command=_getenv("EVOAGENT_REPAIR_TEST_COMMAND", ""),
            repair_verify_timeout_seconds=_int("EVOAGENT_REPAIR_VERIFY_TIMEOUT_SECONDS", 120),
            otel_endpoint=_getenv("EVOAGENT_OTEL_ENDPOINT", ""),
            otel_service_name=_getenv("EVOAGENT_OTEL_SERVICE_NAME", "tracereview"),
            alert_failure_rate=float(_getenv("EVOAGENT_ALERT_FAILURE_RATE", "0.20")),
            alert_min_samples=_int("EVOAGENT_ALERT_MIN_SAMPLES", 10),
            alert_window_seconds=_int("EVOAGENT_ALERT_WINDOW_SECONDS", 900),
            alert_webhook_url=_getenv("EVOAGENT_ALERT_WEBHOOK_URL", ""),
            alert_smtp_host=_getenv("EVOAGENT_ALERT_SMTP_HOST", ""),
            alert_email_to=_getenv("EVOAGENT_ALERT_EMAIL_TO", ""),
            ...
            llm_input_cost_per_million=float(
                _getenv("EVOAGENT_LLM_INPUT_COST_PER_MILLION", "0")
            ),
            llm_output_cost_per_million=float(
                _getenv("EVOAGENT_LLM_OUTPUT_COST_PER_MILLION", "0")
            ),
            evaluation_min_public_prs=_int(
                "EVOAGENT_EVALUATION_MIN_PUBLIC_PRS", 300
            ),
            evaluation_min_f1_improvement=float(
                _getenv("EVOAGENT_EVALUATION_MIN_F1_IMPROVEMENT", "0.03")
            ),
            evaluation_min_high_risk_improvement=float(
                _getenv("EVOAGENT_EVALUATION_MIN_HIGH_RISK_IMPROVEMENT", "0.05")
            ),
        )
