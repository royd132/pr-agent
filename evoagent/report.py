...
def to_markdown(report: Dict[str, Any]) -> str:
    ...
    if collaboration and run_mode.get("effective") == "agentic" and execution.get("llm_calls", 0):
        lines.extend([
            ...
            "- Candidate findings: `%s`; audit decisions: `%s`" % (
                collaboration.get("candidate_findings_before_audit", 0),
                len(collaboration.get("audit_decisions") or []),
            ),
            "",
        ])
