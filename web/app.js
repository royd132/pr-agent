...
let accessToken = localStorage.getItem("tracereview_token") || localStorage.getItem("evoagent_token") || "";
let toastTimer = null;
...
function show(view, updateHash = true) {
  ...
  document.title = `${titles[view]} · TraceReview`;
  ...
  try {
    ...
    if (data.status === "awaiting-approval") {
      ...
      const approved = window.confirm(
        "修复补丁已通过结构检查与配置的测试。是否批准创建 Draft PR？"
      );
      if (approved) {
        setButtonBusy(button, true, "正在发布…");
        const published = await api(
          `/v1/tasks/${encodeURIComponent(selectedTask)}/fix/approve`,
          { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" },
        );
        $("#task-report").textContent = formatJson(published);
        toast("已批准并创建 Draft PR");
      }
    } else {
      toast(data.note || "修复建议已生成");
    }
  } catch (error) {
    toast(error.message);
  } finally {
...
$("#login-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    localStorage.setItem("tracereview_token", accessToken);
    localStorage.removeItem("tracereview_token");
